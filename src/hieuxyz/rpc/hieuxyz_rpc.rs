...
impl From<RpcImage> for ImageSource {
    fn from(o: RpcImage) -> Self {
        ImageSource::Obj(o)
    }
}
...
pub struct HieuxyzRPC {
    ...
    assetMessageIds: Arc<Mutex<HashMap<String, String>>>,
...
impl HieuxyzRPC {
    pub fn new(imageService: ImageService, onUpdate: UpdateCallback) -> Self {
        let mut rpc = Self {
            ...
            assetMessageIds: Arc::new(Mutex::new(HashMap::new())),
    ...
    fn sanitize(&self, str: &str, len: usize) -> String {
        if str.len() > len {
            str[0..len].to_string()
        } else {
            str.to_string()
    ...
    pub fn setName(&mut self, name: &str) -> &mut Self {
        self.activity.name = self.sanitize(name, 128);
        self
    }
    ...
    pub fn setDetails(&mut self, details: &str) -> &mut Self {
        self.activity.details = Some(self.sanitize(details, 128));
        self
    }
    ...
    pub fn setState(&mut self, state: &str) -> &mut Self {
        self.activity.state = Some(self.sanitize(state, 128));
        self
    }
    ...
    pub fn setType(&mut self, t: u8) -> &mut Self {
        self.activity.r#type = t;
        self
    }
    ...
    pub fn setTimestamps(&mut self, start: Option<u64>, end: Option<u64>) -> &mut Self {
    ...
    pub fn setLargeImage<T: Into<ImageSource>>(
        &mut self,
        source: T,
        text: Option<&str>,
    ) -> &mut Self {
    ...
    /// * `source` - Image source (URL, asset key, Asset Name or RpcImage object).
    /// * `text` - Text displayed when hovering over image.
    pub fn setSmallImage<T: Into<ImageSource>>(
        &mut self,
        source: T,
        text: Option<&str>,
    ) -> &mut Self {
    ...
    pub fn addButton(&mut self, label: &str, url: &str) -> &mut Self {
        ...
        if self.activity.metadata.is_none() {
            self.activity.metadata = Some(ActivityMetadata {
                button_urls: Vec::new(),
            });
        }
    ...
    pub fn setButtons(&mut self, buttons: Vec<(String, String)>) -> &mut Self {
        ...
        let labels = valid_buttons
            .iter()
            .map(|b| self.sanitize(&b.0, 32))
            .collect();
    ...
    pub fn setSecrets(
        ...
        self.activity.secrets = Some(ActivitySecrets {
            join,
            spectate,
            r#match: match_secret,
        });
        self
    }
    ...
    pub fn setSyncId(&mut self, syncId: String) -> &mut Self {
        self.activity.sync_id = Some(syncId);
        self
    }
    ...
    /// ```
    pub fn setFlags(&mut self, flags: u32) -> &mut Self {
        self.activity.flags = Some(flags);
        self
    }
    ...
    pub fn setApplicationId(&mut self, id: &str) -> &mut Self {
        ...
        if !is_valid {
            panic!("The app ID must be a valid number string (17-20 digits).");
        }
        self.activity.application_id = Some(id.to_string());
        self
    }
    ...
    pub fn setInstance(&mut self, instance: bool) -> &mut Self {
        self.activity.instance = Some(instance);
        self
    }

    /// Sets the user status (e.g., "online", "dnd", "idle").
    pub fn setStatus(&mut self, status: &str) -> &mut Self {
        self.status = status.to_string();
        self
    }

    fn _resolveAssetUrl(&self, assetKey: &str) -> String {
        if assetKey.starts_with("mp:") {
            return format!("https://media.discordapp.net/{}", &assetKey[3..]);
        }
        if assetKey.starts_with("spotify:") {
            return format!("https://i.scdn.co/image/{}", &assetKey[8..]);
        }
        if assetKey.starts_with("youtube:") {
            return format!("https://i.ytimg.com/vi/{}/hqdefault.jpg", &assetKey[8..]);
        }
        if assetKey.starts_with("twitch:") {
            return format!(
                "https://static-cdn.jtvnw.net/previews-ttv/live_user_{}.png",
                &assetKey[7..]
            );
        }

        if let Some(app_id) = &self.activity.application_id {
            if !assetKey.starts_with("http") {
                return format!(
    ...
    fn getExpiryTime(assetKey: &str) -> Option<u64> {
        if !assetKey.starts_with("mp:attachments") {
            return None;
        }
        ...
        if let Ok(u) = Url::parse(&url_str) {
            if let Some(ex) = u.query_pairs().find(|(k, _)| k == "ex").map(|(_, v)| v) {
                if let Ok(ts) = u64::from_str_radix(&ex, 16) {
                    return Some(ts * 1000);
                }
            }
        }
    ...
    async fn renewAssetIfNeeded(&self, cacheKey: &str, assetKey: &str) -> String {
        let expiry = Self::getExpiryTime(assetKey);
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_millis() as u64;
        if expiry.is_some() && expiry.unwrap() < now + 3600000 {
            let message_id_opt = {
                let map = self.assetMessageIds.lock().unwrap();
                map.get(cacheKey).cloned()
            };
            if let Some(msg_id) = message_id_opt {
                let parts: Vec<&str> = assetKey.split("/").collect();
                if parts.len() >= 4 {
                    let channel_id = parts[1];
                    let raw_filename = parts.last().unwrap();
                    let filename = raw_filename.split('?').next().unwrap_or(raw_filename);
                    let renew_id = format!("{}/{}/{}", channel_id, msg_id, filename);
                    if let Some(new_id) = self.imageService.renewImage(&renew_id).await {
                        self.resolvedAssetsCache
                            .lock()
                            .unwrap()
                            .insert(cacheKey.to_string(), new_id.clone());
                        return new_id;
                    }
                }
            } else {
                logger::warn(&format!(
                    "Cannot renew asset: Message ID missing for {}",
                    cacheKey
                ));
            }
    ...
    fn resolveImage<'a>(&'a self, image: Option<RpcImage>) -> BoxFuture<'a, Option<String>> {
        Box::pin(async move {
            ...
            if cacheKey.starts_with("app_asset:") {
                ...
                logger::warn(&format!(
                    "Asset with name \"{}\" not found for Application ID {}.",
                    name, app_id
                ));
                return None;
            }
            ...
            if let Some(result) = img.resolve(&self.imageService).await {
                let asset_id = result.id;
                if let Some(mid) = result.message_id {
                    self.assetMessageIds
                        .lock()
                        .unwrap()
                        .insert(cacheKey.clone(), mid);
                }
                if asset_id.starts_with("app_asset:") {
                    let recursive_img =
                        RpcImage::Application(asset_id["app_asset:".len()..].to_string());
                    return self.resolveImage(Some(recursive_img)).await;
                }
                self.resolvedAssetsCache
                    .lock()
                    .unwrap()
                    .insert(cacheKey, asset_id.clone());
    ...
    async fn ensureAppAssetsLoaded(&self) {
        if let Some(app_id) = &self.activity.application_id {
            let needs_load = !self
                .applicationAssetsCache
                .lock()
                .unwrap()
                .contains_key(app_id);
            if needs_load {
                logger::info(&format!(
                    "Fetching assets for Application ID: {}...",
                    app_id
                ));
                let assets = self.imageService.fetchApplicationAssets(app_id).await;
                let mut map = HashMap::new();
                for a in assets {
                    map.insert(a.name, a.id);
                }
                let count = map.len();
                self.applicationAssetsCache
                    .lock()
                    .unwrap()
                    .insert(app_id.clone(), map);
                logger::info(&format!(
                    "Loaded {} assets for Application ID: {}.",
                    count, app_id
                ));
            }
        }
    ...
    fn startBackgroundRenewal(&mut self) {
        ...
        let asset_messages = self.assetMessageIds.clone();
                ...
                for (key, assetKey) in entries {
                    if let Some(expiry) = Self::getExpiryTime(&assetKey) {
                        ...
                        if expiry < now + 3600000 {
                            let msg_id_opt = { asset_messages.lock().unwrap().get(&key).cloned() };
                            if let Some(msg_id) = msg_id_opt {
                                let parts: Vec<&str> = assetKey.split("/").collect();
                                if parts.len() >= 4 {
                                    let channel_id = parts[1];
                                    let filename = parts.last().unwrap().split('?').next().unwrap();
                                    let renew_id =
                                        format!("{}/{}/{}", channel_id, msg_id, filename);
                                    if let Some(new_id) = service.renewImage(&renew_id).await {
    ...
    pub async fn buildActivity(&self) -> Option<Activity> {
        ...
        if final_activity.name.is_empty() {
            final_activity.name = "hieuxyzRPC".to_string();
        }
    ...
    pub fn clearCache(&self) {
        ...
        self.assetMessageIds.lock().unwrap().clear();
    ...
    pub fn stopBackgroundRenewal(&mut self) {
        if let Some(t) = &self.renewalTask {
            t.abort();
            self.renewalTask = None;
        }
    }

    pub fn get_currentStatus(&self) -> String {
        self.status.clone()
    }

    pub fn clearDetails(&mut self) -> &mut Self {
        self.activity.details = std::option::Option::None;
        self
    }
    pub fn clearState(&mut self) -> &mut Self {
        self.activity.state = std::option::Option::None;
        self
    }
    pub fn clearTimestamps(&mut self) -> &mut Self {
        self.activity.timestamps = std::option::Option::None;
        self
    }
    pub fn clearParty(&mut self) -> &mut Self {
        self.activity.party = std::option::Option::None;
        self
    }
    pub fn clearButtons(&mut self) -> &mut Self {
        self.activity.buttons = std::option::Option::None;
        self.activity.metadata = std::option::Option::None;
        self
    }
    pub fn clearSecrets(&mut self) -> &mut Self {
        self.activity.secrets = std::option::Option::None;
        self
    }
    pub fn clearInstance(&mut self) -> &mut Self {
        self.activity.instance = std::option::Option::None;
        self
    }
    pub fn clearLargeImage(&mut self) -> &mut Self {
        self.large_image_src = std::option::Option::None;
        self.large_text = std::option::Option::None;
        self
    }
    pub fn clearSmallImage(&mut self) -> &mut Self {
        self.small_image_src = std::option::Option::None;
        self.small_text = std::option::Option::None;
        self
    }
}
